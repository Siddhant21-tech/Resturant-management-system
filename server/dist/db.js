...
class RestaurantDatabase {
    dbPath = path_1.default.join(__dirname, '../dev-db.json');
    restaurants = [];
    branches = [];
    kitchenStations = [];
    tables = [];
    ...
    save() {
        try {
            ...
            fs_1.default.writeFileSync(this.dbPath, JSON.stringify(dump, null, 2));
        }
        catch (e) {
...
exports.prisma = {
    ...
    diningSession: {
        ...
        create: async (args) => {
            ...
            store.save();
            return exports.prisma.diningSession.findUnique({ where: { id: newSession.id } });
        },
        update: async (args) => {
    ...
    bill: {
        ...
        create: async (args) => {
            ...
            store.save();
            return exports.prisma.bill.findUnique({ where: { id: billId } });
        },
        update: async (args) => {
            ...
            if (idx !== -1) {
                ...
                store.save();
                return exports.prisma.bill.findUnique({ where: { id: args.where.id } });
            }
